...
const DynamicWaterMap = dynamic(
  () => import('./WaterMap').then((mod) => mod.WaterMap),
  {
    ssr: false,
    loading: () => (
      <div className="w-full h-full min-h-[420px] rounded-[12px] dark:rounded-[6px] bg-[#f8f9fa] dark:bg-[#141414] border border-[#e0e2e6] dark:border-[#2a2a2a] flex flex-col items-center justify-center text-[#4b5563] dark:text-[#cbd5e1] p-6">
        ...
        <p className="text-xs font-bold text-[#1f2124] dark:text-white font-sans">Loading Geospatial Water Map...</p>
        <p className="text-xs text-[#4b5563] dark:text-[#cbd5e1] mt-1">Initializing Offin, Birim & Ankobra River Basins</p>
      </div>
    ),
  }
);
