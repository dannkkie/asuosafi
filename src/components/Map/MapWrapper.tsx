...
const DynamicWaterMap = dynamic(
  () => import('./WaterMap').then((mod) => mod.WaterMap),
  {
    ssr: false,
    loading: () => (
      <div className="w-full h-full min-h-[420px] rounded-2xl bg-slate-900 border border-slate-800 flex flex-col items-center justify-center text-slate-400 p-6 animate-pulse">
        ...
        <p className="text-xs font-semibold text-slate-300">Loading Geospatial Water Map...</p>
        <p className="text-[11px] text-slate-500 mt-1">Initializing Offin, Birim & Ankobra River Basins</p>
      </div>
    ),
  }
);
